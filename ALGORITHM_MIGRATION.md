...
与单字相同的问题：条边毛刺、模块交界渗墨在干净标签上也产生缺陷，而细小内部缺陷又被边缘排除缩小。现做法：

1. **读码修复**：`ZxingBarcodeDecoder`原图读不出时依次尝试5×5中值、墨迹外扩1像素、2×2降采样，只接受唯一结果；`BarcodeObservation.Preprocessing`记录所用预处理（原图直接读出为空），QR网格按比例换回原图坐标。质量检查据此输出`barcode_decode_assisted`缺陷：内容可信但可读性余量不足。
2. **一维条/空隙**：深度=到本元素左右边缘及上下端的距离，只触及`EdgeTolerance`边缘带的差异不计入，深入内部的按完整面积计入；半宽不超过边缘带的细元素只检查横贯整条宽度的断裂/粘连。
3. **一维扫描等级**（指示性，参照ISO/IEC 15416）：10条扫描线、约0.8倍最窄元素的测量孔径，计算SC、Rmin、ECmin、MOD、缺陷度及条数一致性，F级输出`barcode_scan_grade_fail`缺陷，其余为信息；最窄元素不足4像素不评级。灰度未标定，不是ISO认证。
4. **QR模块**：按模块网格绘制理想图（固定结构用QR规则，数据区用模块多数像素）逐像素比较，深度=到理想黑白交界的距离；边缘带随整体墨迹扩散/收缩加宽（上限为模块半宽40%），深入模块半宽一半以上的差异按连通块完整计入。

实拍5张标签（Code 93两处、QR一处）：干净标签QR缺陷由2–3处降为0，FOSB扫描等级A；局部斑驳标签的QR原图读不出，现经修复读出并报告；FOSB等级随损伤依次为A、A、B、C、F。LOT ID条码最窄元素仅2–3像素，已接近成像分辨率，局部结果只作参考，不评级。
