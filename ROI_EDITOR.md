...
- 矩形/椭圆：左键拖出范围。
- 圆：按下位置为圆心，拖动距离为半径。
- 多边形/折线：逐点单击，Enter、双击或右键单击（不拖动）结束，多边形闭合到首点；Backspace撤销最后一个待定点。
- 编辑：选择工具后拖动形状内部/线，或拖动控制点；旋转柄位于形状上方。
- Esc：取消未提交事务。
- Delete：删除整个选中ROI，不是删除某一个顶点。
- Ctrl+Z：撤销；Ctrl+Y / Ctrl+Shift+Z：重做。
- 中键/右键拖动：平移；滚轮缩放；Home适应。内置视图操作只取消进行中的拖动手势（`CancelDrag`），逐点绘制中的多边形/折线顶点保留；宿主直接修改Viewport前也应先调用`CancelDrag`。
...
读取采用固定类型白名单，不接受运行时类型名。DTD/外部实体禁止，未知版本/坐标系/结构拒绝；XML最多16Mi字符。文本不是合法XML时抛`XmlException`；XML合法但版本、结构、数值或ROI定义无效时统一抛`FormatException`（原因保存在`InnerException`）。配置最多512个ROI、单轮廓4096顶点、总点/游程100000。交互容量或坐标拒绝通过`ValidationError`报告，保留已提交配置。
